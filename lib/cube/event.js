...
exports.getter = function (eventsDB, aggregationsDB) {
    ...
    function getter(request, callback) {
        var stream = !request.hasOwnProperty("stop"),
            delay = request.hasOwnProperty("delay") ? +request.delay : STREAM_DEFAULT_DELAY,
            start = new Date(request.start).getTime(),
            stop = stream ? (new Date(Date.now() - delay)).getTime() : (new Date(request.stop)).getTime(),
        ...
        options = {sort: {_id: -1}, batchSize: 1000};
        ...
        function query(callback) {
            collection.find(match, {}, options, function (error, cursor) {
                ...
                cursor.each(function (error, event) {
                    ...
                    if (event) {
                        callback({id: event._id instanceof mongodb.ObjectID ? undefined : event._id, time: event.t, data: event.d});
                    }
                    else {
                        callback(null);
                    }
        ...
        if (stream && type && streamsBySource) {
            streams = streamsBySource[type];
            ...
            // and moving waiting clients to active clients.
                ...
                if (streamsBySource && type) {
                    streams = streamsBySource[type] = {time: stop, waiting: [], active: [callback]};
                    (function poll() {
                        query(function (event) {
