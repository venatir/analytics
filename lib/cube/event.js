...
exports.putterInit = function (db) {
    ...
    function putter(request, callback) {
        ...
        db.collectionNames("events", {}, function (error, names) {
            ...
            db.createCollection("events", {capped: true, autoIndexId: true, size: eventsSize}, function (err, result) {
                handle(err);
                db.collection("events").ensureIndex({"t": 1}, function (err, result) {
                    handle(err);
                    eventsCollectionCreated = 1;
                    saveEvents();
                });
